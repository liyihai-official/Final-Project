...
switch (opt)
{
  ...
  auto trained_net { final_project::PINN::HeatPINN(3, OUT_SIZE, /* hsize */ 10) };
  trained_net->to(device);

  if (!filename.empty()) torch::load(trained_net, filename);

  auto out = trained_net->forward(val_dataset.X_internal);

...
Similar to how malloc in C and new in C++ require manual memory management, 
...
-a high-level parallelism library—may not be the optimal choice for high-performance applications. 
Therefore, I opted to design a custom MPI environment that encapsulates the necessary features specific to this project.
...
Aligned with polymorphism principles, I designed a template function to retrieve the corresponding MPI basic data types, 
respecting the fundamental data types I defined as traits at the outset.
Besides, I provides some MPI macros in assert file, they provide a unified interface for dealing with MPI-related errors,
ensuring that MPI errors are handling consistently, safely. 


\subsubsection{MPI Topology}
The MPI topology structure Cartesian contains a MPI Communicator and MPI value data type, halo data type,
also, includes a multi-dimension matrix shape object for storing global shape and local shape.
In addition, it has the neighbors' rank in the source and dest sites.
The copy and move constructors, assignment operators are manually removed,
the destructor is customized for free halo data type and cartesian communicator 
due to the MPI security routines.

\paragraph{Determine the local matrix's localtion}
In order to split the $n$ tasks evenly across all $p$ processes.

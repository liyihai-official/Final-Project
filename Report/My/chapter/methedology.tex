...
Such details are defined in the \texttt{assert.hpp}, \texttt{helper.hpp}, \texttt{types.hpp}, 
and other related header files located library and its sub-folder seeing in the appendix \ref{APP:FOLDER}.

\subsection{Template Multi-dimension Matrix Detail Design}
The FDTD method is a type of FDM, and
...
Also, the data types I am using for storing sizes are \texttt{unit32\_t} and \texttt{unit64\_t} while I defined as trait, such
as \texttt{Dworld} and \texttt{Qworld} respectively, 
as \texttt{size\_type} and \texttt{super\_size\_type} in \texttt{\_\_detail} namespace.
...
Although, it is solvable by using \texttt{virtual} inheritance which is called meta-programming, but again, it increase the complexity.
...
Additionally, this object provides member functions \ref{ALG:StrideImplementation} to access the size of a given dimension. 
\begin{algorithm}
  \caption{Stride implementation}
  \label{ALG:StrideImplementation}
  \begin{algorithmic}[1]
    ...
    \STATE \texttt{n = dims.size()}  \hfill \# Store the dimension of matrix.
    \STATE \texttt{strides[d-1] = 1} \hfill \# Stride is 1 in the first dimension.
    \FOR{ \texttt{d = n - 1; d > 0; --d} }     
      \STATE \texttt{strides[d-1] = strides[d] * dims[d]} \hfill \# Determine stride in the latter dimension.
    \ENDFOR
    \RETURN \texttt{strides}
  \end{algorithmic}
\end{algorithm}

\paragraph{Performance Balancing}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE \texttt{FINAL\_PROJECT\_ASSERT\_MSG}                 \hfill \# Number of Arguments must Match the dimension.
    \STATE \texttt{index} = 0, \texttt{i} = 1                \hfill \# Initialize variables in advance.
    \STATE \texttt{indices[] = \_\_shape.check\_and\_cast(ext)}          \hfill \# The indexes must non-negative.
...
When working with the meta-designed multidimensional matrix, we need to utilize the function \texttt{MPI\_Type\_Create\_subarray} 
follow the routine \ref{LST:mpi_subarray} to 
create essential halo datatypes which follows the routine \ref{ALG:CreateHalo} below
\begin{algorithm}
  \caption{Creating MPI Datatype for communication of Ghost}
  \label{ALG:CreateHalo}
  \begin{algorithmic}[1]
  ...
  \end{algorithmic}
\end{algorithm}
Figure \ref{FIG_MPI_3D_exchange_SCHEME} showing the details of communications in 3D Cartesian topology distributed matrices.
For each process, it has to communicate with all the neighbors from three directions, 
sending and receiving data from $6$ other processes which means totally $16$ MPI communications each.
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[scale=0.8, transform shape]
    ...
    % \draw[dashed,->] (0,-2em) -- (0,20em) node[above] {y};
      ...
      \drawcube{(16em, 0em)}{8}
      \drawcube{(0em, 14em)}{8}
      \drawcube{(16em, 14em)}{8}

    \end{tikzpicture}  
  \caption{
    Representation of a 3D MPI Communication Scheme of $4$ among $24$ processes between $3$ dimension sub-arrays.
    The reddish small cube stands for the memory space stored the ghost data that needed for MPI communications.
    The bluish small cube stands for the space stored the junk values which are not gonna be used in the entire progress.
    The source/destination directions show the relationships between local process and other neighbor processes.
  }
...
Besides that, we could see the figure \ref{FIG_MPI_3D_exchange_SCHEME} shows the 
bluish cube which stands for the memory space for storing junk values.
As the sub-array of each process gets smaller, the percentage of junk values goes up which means updating 
the values in this array is a low-efficient operation.
...
the MPI type create function 
\texttt{MPI\_Type\_create\_subarray} was used for creating newtype for creating file views.
...
member variable \texttt{std::array<Bool, 2*NumD> isDirchletBC} and other one for 
\texttt{std::array<Bool, 2*NumD> isNeumannBC}.
...
%     % \node[draw, rectangle, minimum size=1em] (A) {Hybrid Platforms};
%     % \node[draw, rectangle, below of=A, minimum size=1em] (B) {hybrid MPI + OpenMP};
%     % \node[draw, rectangle, below of=B, minimum size=1em] (C) {pure MPI};
%     % \node[draw, rectangle, right of=, minimum size=1em] (C) {C};
...
\begin{algorithm}
  \caption{Master-only Hybrid with non-overlapped Communication/Computation}
  \label{ALG:Hybrid_0}
  \begin{algorithmic}[1]
    \FOR {iterations}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=0.8, transform shape]
    ...
    % \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=0.5\textwidth]{figure/FIG_Schematic_Hybrid0.pdf}};
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=0.58\textwidth]{figure/FIG_Schematic_Hybrid0.pdf}};

    \begin{scope}[x={(image.south east)},y={(image.north west)}]

    % Draw a grid with steps of 1 cm
    % \def\gridsize{1};
    % \draw[help lines, step=0.1] (0,0) grid (\gridsize,\gridsize);
  
    % Draw axes
    % \draw[thick,->] (0,0) -- (\gridsize,0) node[right] {$x$};
    % \draw[thick,->] (0,0) -- (0,\gridsize) node[above] {$y$};
    \node at (0.5, 1.05) {MPI Initialization};
    \node at (1.2, 0.9) {OpenMP parallel};
        
    \node at (1.2, 0.6) {OpenMP single};
    \node at (0.5, 0.45) {MPI Communication};
    \node at (1.2, 0.1) {OpenMP single};

    \node at (0.13, 0.2) {Update Grid};
    \node at (0.46, 0.2) {Update Grid};
    \node at (0.86, 0.2) {Update Grid};

    \node at (0.13, 0.7) {Update Grid};
    \node at (0.46, 0.7) {Update Grid};
    \node at (0.86, 0.7) {Update Grid};

    \end{scope}
  \end{tikzpicture}
  \caption{
      Schematic view of the master only hybrid strategy with non-overlapping of computation and communication (comm./comp.).
      Using OpenMP barrier pragma for threads barrier to ensure all grids are updated. 
      Then applied single for applying MPI communications, exchanging ghost data.
  }
...
Such strategy is called Master-Only Hybrid 
                                                 ,Page 101 in \cite{openmp-hybrid-mpi-openmp}, 
it can be using \texttt{omp single} for calling MPI communications
rather than using \texttt{omp master}.
In general the master-only hybrid has such communication scheme shown in figure \ref{FIG:master_only_hybrid}.
In the figure, we could clearly see that the details of cooperations between OpenMP and MPI.
Using OpenMP single is the easiest way and an efficient method for hybrid communications, 
due to the \texttt{master} requires the root threads ( thread id is 0) available for continue working.
...
\begin{algorithm}
  \caption{Funneled Master-only Hybirid with overlapped Communication/Computation}
  \label{ALG:Hybrid_1}
  \begin{algorithmic}[1]
    \FOR {iterations}
      \STATE \texttt{\#pragma omp single}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=0.7, transform shape]
    ...
    % \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=0.35\textwidth]{figure/FIG_Schematic_Hybrid1.pdf}};
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=0.48\textwidth]{figure/FIG_Schematic_Hybrid1.pdf}};
    ...
    \node at (0.5, 1) {MPI Initialization};
    \node at (1.25, 0.9) {OpenMP parallel};
        
    \node at (1.25, 0.6) {OpenMP single};
    \node at (0.5, 0.46) {MPI Communication};
    \node at (1.25, 0.1) {OpenMP single};

    \node at (0.16, 0.21) {Update Bulk};
    \node at (0.5, 0.21) {Update Edge};
    \node at (0.84, 0.21) {Update Bulk};

    \node at (0.16, 0.7) {Update Bulk};
    \node at (0.5, 0.7) {Update Edge};
    \node at (0.84, 0.7) {Update Bulk};
  ...
  \end{tikzpicture}
  \caption{
  Scheme: Master only hybrid strategy with computation/communication overlapping of computation and communication (comm./comp.).
    The OpenMP threads are separated into two parts, one of which will do the MPI non-blocking communication and the other threads will 
    update the bulk.
    Using MPI wait all to ensure all threads are available, then update the edges.
  }
...
Totally, the loss function follows the setting in section \ref{SEC:PINN} which is the summation of $loss_{PDE}$ and $loss_{NN}$ where 
\begin{equation}
  \begin{cases}
  ...
  \end{cases}
\end{equation}
In this case, computing the derivative of a output from a large model is expensive in old days.
However, with the benefits from GPU parallel and Auto-differentiation \cite{AD}, we are able to compute the derivatives and 
treat them as regularizer of neural networks. 

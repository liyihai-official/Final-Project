...
int main(int argc, char ** argv)
{
  ...
  if (world.rank() == root) 
  {
    ...
    store_Array(gather, "output_parO3.dat");
    std::cout << "Parallel : " 
    << (t2 - t1) * 1000
    << " ms\n" << std::endl;
  }
                          ...
                          MPI_Comm comm)
{
  ...
  MPI_Type_vector(count, block_length, MAX_N, MPI_DOUBLE, &block);
  ...
  if (rank == root)
  {
    for (pid = 0; pid < size; ++pid) 
    {
      ...
      {
        tag = pid;
        MPI_Type_vector(counts[pid], block_lengths[pid], MAX_N, MPI_DOUBLE, &temp);
